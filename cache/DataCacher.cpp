...
std::vector<PlayFieldFilled> DataCacher::RemoveExtraLines(std::vector<PlayFieldFilled> FilledFields) { //removes full lines of blank, they shouldn't be there for extension
...
//void DataCacher::Test_RemoveExtraLines() {
...
std::vector<std::vector<int>> DataCacher::RecurseInsertLines(int MaxNum, int SelectNum, int LengthLeft) { //maxnum starts with 1, selectnum starts with 0
...
std::vector<std::vector<int>> DataCacher::ReturnInsertLines(int MaxNum, int Length) { //sort of over-engineered this
	std::vector<std::vector<int>> Output;
...
//void DataCacher::Test_ReturnInsertLines() { //test the funcion of ReturnInsertLines
...
/*std::vector<std::vector<PlayFieldFilled>> DataCacher::ReturnAllRotations() {
...
std::vector<DataCacher::TCacheItem> DataCacher::ReturnAllRotations_2() {
	//using tetrominodata, return all possible rotation states
...
/*std::vector<PlayFieldFilled> DataCacher::ExpandToNHigh(std::vector<PlayFieldFilled> FilledFields, int MaxHeight) { //find height of the playfield, insert blanks with combinatorics in mind
	std::vector<PlayFieldFilled> Output;
	for (auto FilledField : FilledFields) {
...
std::vector<DataCacher::TCacheItem> DataCacher::ExpandToNHigh_2(std::vector<TCacheItem> CacheItems, int MaxHeight) { //find height of the playfield, insert blanks with combinatorics in mind
	std::vector<TCacheItem> Output;
...
//void DataCacher::Test_ReturnData() { //return all fields with inserted lines for 4 lines and tetrominos
...
/*std::vector<PlayFieldFilled> DataCacher::ReturnShiftedPossibilities(std::vector<PlayFieldFilled> FilledFields, int MaxWidth) {
	std::vector<PlayFieldFilled> Output;
...
std::vector<DataCacher::TCacheItem> DataCacher::ReturnShiftedPossibilities_2(std::vector<TCacheItem> CacheItems, int MaxWidth) {
	std::vector<TCacheItem> Output;
	for (auto CacheItem : CacheItems) {
...
std::vector<std::vector<int>> DataCacher::ReturnInvolvedRow(std::vector<TCacheItem> CacheItems) {
	std::vector<std::vector<int>> Output = {};
	...
	for (size_t CacheItem = 0; CacheItem < CacheItems.size(); CacheItem++) {
		...
		for (size_t i = 0; i < RowFilled.size(); i++) {
			...
			Output[i].push_back((int)CacheItem);
		}
	}
	return Output;
}
...
DataCacher::TCache DataCacher::ReturnData_2(int height, int width) {
	auto StartTime = std::chrono::system_clock::now();
	TCache Output;
